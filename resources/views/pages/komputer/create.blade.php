...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-5 text-center">Tambah Komputer</h4>
                    <form action="{{ route('komputer.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class='form-group mb-3'>
                            ...
                            <textarea name='spesifikasi' id='spesifikasi' cols='30' rows='3'
                                class='form-control @error('spesifikasi') is-invalid @enderror'>{{ old('spesifikasi') }}</textarea>
                            @error('spesifikasi')
                                <div class='invalid-feedback'>
                                    {{ $message }}
                                </div>
                        ...
                        <div class='form-group'>
                            ...
                            <select name='sistem_operasi_id' id='sistem_operasi_id'
                                ...
                                @foreach ($data_sistem_operasi as $sistem_operasi)
                                    <option @if ($sistem_operasi->id == old('sistem_operasi_id')) selected @endif
                        ...
                        <div class='form-group'>
                            ...
                            <select name='ip_address_id' id='ip_address_id'
                                ...
                                @foreach ($data_ip_address as $ip)
                                    <option @if ($ip->id == old('ip_address_id')) selected @endif value='{{ $ip->id }}'>
                                        {{ $ip->ip }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group text-right">
                            ...
                            <button class="btn btn-primary">Tambah Komputer</button>
                        </div>
                    </form>
                </div>
            </div>
